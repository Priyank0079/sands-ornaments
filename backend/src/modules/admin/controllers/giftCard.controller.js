...
const VALID_STATUSES = ["pending", "active", "partially_used", "used", "expired", "disabled"];
...
exports.getAllGiftCards = async (req, res) => {
  try {
    ...
    if (email)  query.recipientEmail = { $regex: String(email).trim(), $options: "i" };
    if (search) {
    ...
    const allowedSort = new Set(["createdAt", "updatedAt", "value", "balance", "status"]);
    ...
    return success(res, {
      giftCards,
      pagination: { total, page: safePage, limit: safeLimit, pages: Math.ceil(total / safeLimit) || 1 },
...
exports.issueGiftCard = async (req, res) => {
  try {
    const { value, recipientName, recipientEmail, senderName, personalMessage, expiresAt, sendEmail: doSendEmail = true } = req.body;

    if (!value || Number(value) < 1)    return error(res, "Value must be at least ₹1", 400);
    ...
    if (!code) return error(res, "Failed to generate a unique gift card code. Please try again.", 500);
    ...
    const card = await GiftCard.create({
      ...
      purchasedByName:   "Sands Ornaments (Admin)",
    ...
    return success(res, { giftCard: card }, "Gift card issued successfully", 201);
  } catch (err) { return error(res, err.message); }
};
...
exports.toggleDisable = async (req, res) => {
  try {
    const card = await GiftCard.findById(req.params.id);
    if (!card) return error(res, "Gift card not found", 404);
    if (card.status === "used") return error(res, "Cannot modify a fully used gift card", 400);

    const nextStatus = card.status === "disabled" ? "active" : "disabled";
    card.status = nextStatus;
    await card.save();
    return success(res, { giftCard: card }, `Gift card ${nextStatus === "disabled" ? "disabled" : "re-enabled"}`);
  } catch (err) { return error(res, err.message); }
};
...
exports.adjustBalance = async (req, res) => {
  try {
    ...
    if (!Number.isFinite(bal) || bal < 0) return error(res, "Balance must be a non-negative number", 400);

    const card = await GiftCard.findById(req.params.id);
    if (!card) return error(res, "Gift card not found", 404);
    if (card.status === "disabled") return error(res, "Cannot adjust balance of a disabled card", 400);
    ...
    await card.save();
    return success(res, { giftCard: card }, `Balance adjusted from ₹${prevBalance} to ₹${bal}`);
  } catch (err) { return error(res, err.message); }
};
...
exports.deleteGiftCard = async (req, res) => {
  try {
    ...
    if (card.status !== "pending" && card.redemptions.length > 0) {
      return error(res, "Cannot delete a gift card that has been redeemed. Disable it instead.", 400);
    }
