...
const WomenFeaturedProductsEditor = ({ sectionData, onSave, defaultSection = {} }) => {
    ...
    const initialSettings = useMemo(() => {
        ...
        return {
            title: current.title || fallback.title || "Women's Exclusives",
            productLimit: String(parsePositiveNumber(current.productLimit || fallback.productLimit, 8)),
            sourceMode: mergedSourceMode === 'manual' ? 'manual' : 'category',
            categoryId: String(categoryHint || ''),
            ctaLabel: current.ctaLabel || fallback.ctaLabel || "Explore All Women's Jewellery"
        };
    ...
    const handleSave = async () => {
        const productLimit = parsePositiveNumber(settings.productLimit, 8);
        ...
        try {
            await onSave({
                settings: {
                    title: String(settings.title || '').trim() || "Women's Exclusives",
                    productLimit,
                    sourceMode,
                    categoryId: sourceMode === 'category' ? String(settings.categoryId || '').trim() : '',
                    ctaLabel: String(settings.ctaLabel || '').trim() || "Explore All Women's Jewellery"
                },
                items: payloadItems
            });
        } finally {
    ...
    return (
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 md:px-8 py-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h3 className="text-xl md:text-2xl font-bold text-[#3E2723]">Women&apos;s Exclusives</h3>
                    <p className="text-sm text-gray-500 mt-1">
                        Configure this section from CMS. Products are always rendered women-focused on user side.
                    </p>
                </div>
                <button
                    type="button"
            ...
            <div className="p-6 md:p-8 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input
                        ...
                        placeholder="Women's Exclusives"
                    />
                    <Input
                        label="Product Count"
                        type="number"
                ...
                <Input
                    ...
                    onChange={(event) => handleSettingChange('ctaLabel', event.target.value)}
                    placeholder="Explore All Women's Jewellery"
                />
            </div>
...
export default WomenFeaturedProductsEditor;
